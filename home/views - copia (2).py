...
def Login(request):
	if request.method == 'POST':
		url = "https://apimusic.pythonanywhere.com/bar/Verified_Login/"
		payload = json.dumps({
...
def Get_Gender_Bar(code):
	global gender
	url = "https://apimusic.pythonanywhere.com/bar/Get_Gender_Bar/"
	payload = json.dumps({
	  "code": code
	})
	headers = {
...
def Index(request):
	...
	url = "https://apimusic.pythonanywhere.com/bar/Get_List_Song/"
	payload = json.dumps({
...
def Save_Music_Client(request):
	if request.is_ajax():
		...
		url = "https://apimusic.pythonanywhere.com/bar/Save_Song/"
		payload = json.dumps({
		  "code": request.session['code'],
		  "music": title,
...
def Download_Musics(request):
	...
	for i in range(1):
	    ...
	    title = str(videosSearch.result()['result'][i]['title']).replace('"','_').replace('´','')
